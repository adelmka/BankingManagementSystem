...
def commands(account_service, input_handler, menu_renderer, logger):
    with patch("cli.commands.account_commands.get_logger", return_value=logger):
        return AccountCommands(account_service, input_handler, menu_renderer)


def money(value: str) -> Money:
    return Money(Decimal(value), "SAR")
...
def test_create_account_builds_savings_account_and_opens_it(commands, account_service, input_handler, menu_renderer):
    input_handler.get_value.side_effect = ["C000001", "savings", "0.035", "monthly"]
    input_handler.get_money.side_effect = [money("1000.00"), money("500.00")]
    account_service.open_account.side_effect = lambda account: account

    with patch("cli.commands.account_commands.IDGenerator.account_number", return_value="A000001"):
    ...
    assert account.minimum_balance == money("500.00")
    assert account.interest_frequency is InterestFrequency.MONTHLY
    assert account.balance == money("1000.00")
    menu_renderer.display_message.assert_called_once_with("Account created successfully.")
    menu_renderer.display_object.assert_called_once_with(account)


def test_create_account_builds_current_account_and_opens_it(commands, account_service):
    commands.input_handler.get_value.side_effect = ["C000001", "current"]
    commands.input_handler.get_money.side_effect = [
        money("1000.00"), money("5000.00"), money("25.00"), money("50.00")
    ]
    commands.input_handler.get_confirmation.return_value = True
    account_service.open_account.side_effect = lambda account: account

    with patch("cli.commands.account_commands.IDGenerator.account_number", return_value="A000002"):
        commands.create_account()

    account = account_service.open_account.call_args.args[0]
    assert isinstance(account, CurrentAccount)
    assert account.overdraft_limit == money("5000.00")
    assert account.maintenance_fee == money("25.00")
    assert account.overdraft_fee == money("50.00")
    assert account.overdraft_enabled is True


def test_create_account_builds_time_deposit_and_opens_it(commands, account_service):
    commands.input_handler.get_value.side_effect = [
        "C000001", "time deposit", "0.045", "12", "0.02"
    ]
    commands.input_handler.get_money.return_value = money("10000.00")
    commands.input_handler.get_confirmation.return_value = False
    account_service.open_account.side_effect = lambda account: account

    with patch("cli.commands.account_commands.IDGenerator.account_number", return_value="A000003"):
        commands.create_account()
...
def test_create_account_handles_error(commands, account_service, menu_renderer, logger):
    commands.input_handler.get_value.side_effect = ["C000001", "savings", "0.035", "monthly"]
    commands.input_handler.get_money.side_effect = [money("1000.00"), money("500.00")]
...
def test_deposit_delegates(commands, account_service, menu_renderer):
    ...
    amount = money("250.00")
    commands.input_handler.get_money.return_value = amount
    account = MagicMock(name="account")
    account_service.deposit.return_value = account

    commands.deposit()

    account_service.deposit.assert_called_once_with("A000001", amount)
    menu_renderer.display_object.assert_called_once_with(account)
...
def test_withdraw_delegates(commands, account_service, menu_renderer):
    ...
    account_service.withdraw.assert_called_once_with("A000001", amount)
    menu_renderer.display_object.assert_called_once_with(account)
