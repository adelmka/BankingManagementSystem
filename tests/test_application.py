...
These tests target the current Application public contract without
constructing the real DependencyContainer or BankService.
"""
...
@pytest.fixture
def container():
    """Return a mocked dependency container."""
...
@pytest.fixture
def application(monkeypatch, config, container, logger):
    """Return an Application using isolated mocked dependencies."""
    monkeypatch.setattr(
...
def test_default_config_is_used_when_no_config_is_supplied(monkeypatch):
    container = MagicMock(name="container")
...
def test_supplied_config_is_retained(application, config):
    assert application.config is config
...
def test_initialization_logs_completion(application, logger):
    logger.info.assert_any_call("Application initialized.")


def test_bank_returns_container_bank_service(application, container):
    assert application.bank is container.bank_service


def test_bank_returns_same_facade_each_time(application):
    assert application.bank is application.bank


def test_config_returns_supplied_config(application, config):
    assert application.config is config
...
def test_shutdown_delegates_to_container(application, container):
...
def test_shutdown_logs_completion(application, container, logger):
    container.shutdown.reset_mock()
...
def test_shutdown_can_be_called_more_than_once(application, container):
    container.shutdown.reset_mock()
...
def test_initialization_propagates_container_build_exception(
    ...
    monkeypatch.setattr(
        "application.application.DependencyContainer.build",
        MagicMock(side_effect=RuntimeError("build failed")),
    )
...
def test_is_running_propagates_validation_exception(application, container):
    container.validate.reset_mock()
    container.validate.side_effect = RuntimeError("validation failed")

    with pytest.raises(RuntimeError, match="validation failed"):
        _ = application.is_running


def test_repr_uses_class_name_and_running_state(application, container):
    container.validate.reset_mock()
    container.validate.return_value = True
...
def test_repr_reflects_not_running_state(application, container):
    container.validate.reset_mock()
    container.validate.return_value = False
...
def test_str_returns_application_name(application, config):
...
def test_application_retains_private_logger_reference(application, logger):
    assert application._logger is logger


def test_application_initialization_order_creates_logger_before_container(
    monkeypatch,
    config,
    container,
):
...
def test_application_is_independent_of_bank_service_type(
    application,
    container,
):
